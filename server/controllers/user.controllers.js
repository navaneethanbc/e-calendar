...
import { validateLogin, validateRegister, validateResetPassword } from "../utils/userValidator.js";
...
export const sendOtp = async(req,res)=>{
  try{
    ...
    res.status(200).send({ message: "OTP generated successfully check your email" })
...
export const verifyOtp = async(req,res)=>{
  try{
    ...
    if(!user.resetToken || user.resetTokenExpires < Date.now()){
      return res.status(400).send({message:"your otp is invalid or expired"})
    }
...
export const resetPassword = async (req,res)=>{
  try{
    ...
    if(!user.otpVerified){
      return res.status(400).send({message:"otp verification required to change the password"})
    }
